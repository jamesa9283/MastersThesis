...
\noindent\fbox{%
    ...
    \noindent
      ...
      \begin{ndefi}[Vector Bundle]
        A $\mathbbm{k}$-vector bundle over $M$ of rank $k$ consists of; a bundle $\pi : E \to M$ whose fibers are $\mathbbm{k}$-vector spaces and around each point $p \in M$, there is some open $U \subset M$ and a diffeomorphism $\Phi : U \ti \mathbbm{k}^k \to {\left. E \right|_U}$ such that,
        \begin{itemize}
